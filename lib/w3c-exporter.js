...
/**
 * Convert color value to W3C color format
 */
function hexToW3CColor(color) {
  let r, g, b;
  ...
  if (typeof color === 'string' && color.includes('rgb')) {
    ...
    if (match) {
      ...
      // Convert to hex for the hex field
    ...
    .forEach((entry, index) => {
      ...
      shadowTokens[name] = {
        ...
        $value: {
          ...
          color: parts[4] && parts[4].match(/^#[0-9a-fA-F]{6}$/)
            ? hexToW3CColor(parts[4])
            : hexToW3CColor('#000000')
        }
      };
    });
